...
const contractJsonPath = path.resolve(__dirname, '../','contract/storage/bin','Storage.json');
console.log('contractJsonPath:',contractJsonPath)
...
async function createContract(host,privatekey) {
  const web3 = new Web3(host);
...
// console.log("gas:", gas);
  const txn = {
    chainId: 1337,
    nonce: nonce,       // 0x00 because this is a new account
    from: account.address,
    to: null,            //public tx
    value: "0x00",
    data: '0x'+contractBytecode,
    ...
    //max number of gas units the tx is allowed to use
  };
...
async function main(){
  createContract(host,privatekey)
